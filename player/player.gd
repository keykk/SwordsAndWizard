...
func _ready():
	...
	sprite = GameManager.find_node_of_type(self,"AnimatedSprite2D","AnimatedSprite2D")
	if not sprite:
		sprite = GameManager.find_node_of_type(self,"Sprite2D","Sprite2D")
	#if $Sprite2D:
...
func rotate_sprite() -> void:
	# Girar sprite
	if input_vector.x > 0 and sprite:
...
func attack() -> void:
	...
	# Cooldown
	attack_cooldown = animation.get_length()#0.6
	#Marcar ataque
	is_attacking = true
